...
face = cv.CascadeClassifier(cv.data.haarcascades + 'haarcascade_frontalface_alt.xml')
leye = cv.CascadeClassifier(cv.data.haarcascades + 'haarcascade_lefteye_2splits.xml')
reye = cv.CascadeClassifier(cv.data.haarcascades + 'haarcascade_righteye_2splits.xml')

label = ['Closed','Open']

model = load_model('Trainer/eye_detection.h5')
path = os.getcwd()
...
while(True):
    ...
    for (x, y, w, h) in right_eye:
        ...
        rightPred = np.argmax(model.predict(r_eye), axis=-1)
    ...
    for (x, y, w, h) in left_eye:
        ...
        leftPred = np.argmax(model.predict(left_eye), axis=-1)
